...
function draw() {
    ...
    for (var i = 1; i < circles.length; i++) {
        circles[i].radius = dataArray[i] * 2;
        circles[i].y = circles[i].y > canvas.height ? 0 : circles[i].y + 1;
        circles[i].draw();
    }
    
    for (var i = 1; i < dataArray.length; i += 10){
        ctx.fillStyle = 'rgb(' + getRandomColor() + ',' + getRandomColor() + ',' + getRandomColor() + ')';
        ctx.fillRect(i + 300, canvas.height - dataArray[i] * 1.5, 10, canvas.height);
        ctx.strokeRect(i + 300, canvas.height - dataArray[i] * 1.5, 10, canvas.height);
    }
}
